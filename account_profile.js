...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const organizationAccountData = {
        "Tech Solutions Inc.": {
            ...
            transactions: [
                { 
                    id: 101, campaign: '2025 Workplace Campaign', amount: 15000.00, paid: 7450.00, date: '2025-09-01', 
                    designations: [{agency: 'RUM', amount: 10000}, {agency: 'Community Shelter', amount: 5000}], 
                    recurring: null,
                    details: {
                        'Solicitor': 'John Doe (Dev)', 'Soft Credit': 'Alex Doe', 'Pledge Type': 'Workplace Campaign',
                        'Source': 'Corporate Portal', 'Batch Number': 'WP-2025-09-01', 'Date Received': '2025-08-31',
                    },
                    payments: [
                        { id: 'pay-001', date: '2025-09-15', amount: 7500.00, method: 'Payroll Deduction', reference: 'PAY-SEP-2025', processedBy: 'System' }
                    ],
                    adjustments: [
                        { id: 'adj-001', date: '2025-10-01', amount: -50.00, reason: 'Pledge Correction', adjustedBy: 'Admin User' }
                    ],
                    documents: [
                    ...
                    auditLog: [
                        ...
                        { user: 'Admin User', action: 'Adjustment of -$50.00 Posted', date: '2025-10-01', icon: 'fa-pencil', color: 'bg-orange-500' },
                    ]
                },
                { 
                    ...
                    recurring: null,
                    details: { 'Solicitor': 'John Doe (Dev)', 'Pledge Type': 'Workplace Campaign', 'Source': 'Corporate Portal', 'Batch Number': 'WP-2024-09-01' },
                    payments: [ { id: 'pay-002', date: '2024-09-15', amount: 12500.00, method: 'Payroll Deduction', reference: 'PAY-SEP-2024', processedBy: 'System' } ],
                    adjustments: [],
                    documents: [ { id: 'doc3', name: '2024 Pledge Form.pdf', content: 'This is the content of the 2024 pledge form PDF.' } ],
                    auditLog: [
            ...
            campaignHistory: [
                { year: 2025, employees: 120, donors: 85, participation: '71%', amount: 15000.00 },
                { year: 2024, employees: 115, donors: 78, participation: '68%', amount: 12500.00 },
                { year: 2023, employees: 110, donors: 70, participation: '64%', amount: 10000.00 },
            ],
            engagements: {
                events: [ { eventName: 'Annual Charity Gala Sponsorship', date: '2025-08-15', status: 'Confirmed' } ],
                volunteering: [ { opportunityName: 'Day of Caring 2025', agency: 'Community Shelter', date: '2025-06-05', status: 'Completed', hours: 80 } ]
            },
            communications: {
                logs: [ { id: 10, type: 'Meeting', subject: 'Workplace Campaign Kickoff', sender: 'John Doe (Dev)', date: '2025-08-15', content: 'Met with HR to plan the 2025 campaign kickoff event.' } ],
                notes: [ { id: 11, title: 'Key Contact Update', date: '2025-04-02', content: 'Alex Doe is the new primary contact for the campaign.' } ],
                documents: []
            }
        },
        "Innovate Corp": {
            ...
            contacts: [ { name: 'Jane Smith', role: 'CEO', email: 'jane.smith@innovate.corp' } ],
            transactions: [ { id: 103, campaign: '2025 Sponsorship', amount: 10000.00, paid: 10000.00, date: '2025-01-20', designations: [], recurring: null, documents: [], auditLog: [], details: {}, payments: [], adjustments: [] } ],
            campaignHistory: [], engagements: { events: [], volunteering: [] }, communications: { logs: [], notes: [], documents: [] }
        },
         "Anytown Community Foundation": {
            ...
            contacts: [ { name: 'Director of Grants', role: 'Primary Contact', email: 'grants@anytowncf.org' } ],
            transactions: [],
            campaignHistory: [], engagements: { events: [], volunteering: [] }, communications: { logs: [], notes: [], documents: [] }
        }
    };
    ...
    function createDetailsModalHtml(data) {
    ...
    const renderers = {
        profile: (container, data) => {
            ...
            container.querySelector('#account-name + p + p').classList.add('hidden');
        ...
        campaign: (container, data) => {
            ...
            const historyContainer = container.querySelector('#campaign-history-container');
            if (historyContainer) {
                 if (campaignHistory.length === 0) {
                    historyContainer.innerHTML = '<tr><td colspan="5" class="text-center py-4 text-gray-500">No campaign history found.</td></tr>';
                } else {
                    historyContainer.innerHTML = campaignHistory.map(row => `
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            <td class="px-4 py-3 font-semibold">${row.year}</td>
                            <td class="px-4 py-3 text-center">${row.employees}</td>
                            <td class="px-4 py-3 text-center">${row.donors}</td>
                            <td class="px-4 py-3 text-center">${row.participation}</td>
        ...
        engagements: (container, data) => {
            ...
            renderEngagementList(container, '#event-attendance-container', engagements.events, 'event');
            renderEngagementList(container, '#volunteer-history-container', engagements.volunteering, 'volunteer');
        },
        communications: (container, data) => {
            const comms = data.communications || { logs: [], notes: [], documents: [] };
            renderCommList(container, '#communications-log-container', comms.logs);
            renderNotesList(container, '#notes-container', comms.notes);
            renderDocsList(container, '#documents-container', comms.documents);
        }
    };
    
    function renderEngagementList(container, listId, data, type) {
        const listContainer = container.querySelector(listId);
        if (!listContainer) return;
        if (data.length === 0) {
            listContainer.innerHTML = `<div class="text-center py-8 text-gray-500">No ${type} history found.</div>`;
            return;
        }
        const statusStyles = { 'Confirmed': 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300', 'Completed': 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' };
        const iconInfo = { event: { icon: 'fa-calendar-star', bg: 'bg-purple-100 text-purple-600 dark:bg-purple-900/50 dark:text-purple-400' }, volunteer: { icon: 'fa-hands-holding-child', bg: 'bg-green-100 text-green-600 dark:bg-green-900/50 dark:text-green-400' }};
    ...
    function renderCommList(container, listId, logs) {
        const logContainer = container.querySelector(listId);
        if(!logContainer) return;
        if(logs.length === 0) { logContainer.innerHTML = `<tr><td colspan="4" class="text-center py-8 text-gray-500">No communication logs found.</td></tr>`; return; }
    ...
    function renderNotesList(container, listId, notes) {
        const notesContainer = container.querySelector(listId);
        if(!notesContainer) return;
        if(notes.length === 0) { notesContainer.innerHTML = `<div class="text-center py-4 text-gray-500 text-sm">No notes found.</div>`; return; }
        notesContainer.innerHTML = notes.map(item => `
    ...
    function renderDocsList(container, listId, documents) {
        const docsContainer = container.querySelector(listId);
        if(!docsContainer) return;
        if(documents.length === 0) { docsContainer.innerHTML = `<div class="text-center py-4 text-gray-500 text-sm">No documents found.</div>`; return; }
        ...
        docsContainer.innerHTML = documents.map(item => `
    ...
    function initGivingChart(canvas) {
        ...
        new Chart(canvas, { type: 'bar', data: { labels: ['2023', '2024', '2025'], datasets: [{ label: 'Workplace Giving', data: [10000, 12500, 15000], backgroundColor: gradient, borderColor: `hsl(${primaryColor})`, borderWidth: 1, borderRadius: 4 }] }, options: { responsive: true, maintainAspectRatio: false, barPercentage: 0.5, categoryPercentage: 0.6, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { color: labelColor, callback: (value) => '$' + value / 1000 + 'k' }, grid: { color: gridColor } }, x: { ticks: { color: labelColor }, grid: { display: false } } } } });
    }
    ...
    function initDesignationChart(canvas, dataType = 'priorYear') {
        ...
        const data = {
            priorYear: { labels: ['RUM', 'Community Shelter'], data: [10000, 5000] },
            lifetime: { labels: ['RUM', 'Community Shelter'], data: [22500, 5000] }
        };
        designationChart = new Chart(canvas, { type: 'doughnut', data: { labels: data[dataType].labels, datasets: [{ data: data[dataType].data, backgroundColor: backgroundColors, borderColor: isDarkMode ? '#1f2937' : '#fff', borderWidth: 2 }] }, options: { responsive: true, maintainAspectRatio: false, cutout: '70%', plugins: { legend: { position: 'right', labels: { color: labelColor, boxWidth: 12, padding: 20 } } } } });
    }
    ...
    accountProfileContainer.addEventListener('click', (e) => {
        ...
        if (viewDetailsButton) {
            ...
            const modalHtml = createDetailsModalHtml(detailsData);
