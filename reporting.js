document.addEventListener('DOMContentLoaded', () => {
    ...
    const initializeDrawerUI = (container) => {
        ...
        campaignYearDropdowns.forEach(dropdown => {
            if (dropdown.options.length === 1) { // Only populate once
        ...
        multiSelectButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                if (modal) {
                    modal.style.zIndex = '60'; // This ensures the modal is on top of the drawer
    ...
    // --- NEW: Campaign Account Modal Logic ---
    const campaignAccountModal = getEl('campaign-account-modal');
    if (campaignAccountModal) {
        const checkboxContainer = getEl('campaign-account-checkboxes');
        ...
        mockCampaigns.forEach(name => {
             checkboxContainer.innerHTML += `<label class="checkbox-label col-span-2"><input type="checkbox" class="checkbox" value="${name}"><span>${name}</span></label>`;
        });

        getEl('campaign-account-modal-done').addEventListener('click', () => {
            const selected = Array.from(checkboxContainer.querySelectorAll('input:checked'));
             document.querySelectorAll('[data-modal-target="campaign-account-modal"]').forEach(button => {
                button.textContent = selected.length > 0 ? `${selected.length} Campaigns Selected` : 'No items selected';
                button.classList.toggle('active', selected.length > 0);
            });
            campaignAccountModal.style.display = 'none';
            campaignAccountModal.style.zIndex = ''; // Reset the z-index when the modal closes
        });

         campaignAccountModal.addEventListener('click', (e) => {
             if (e.target === campaignAccountModal) getEl('campaign-account-modal-done').click();
         });
    }
